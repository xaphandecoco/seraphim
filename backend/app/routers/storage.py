...
def _authenticate(request: Request) -> bool:
    ...
    <img> tags send cookies automatically (same-origin), so the refresh cookie is
    the correct auth mechanism for browser image requests. Bearer is used by the
    Axios client and Postman.
    """
    ...
    # 2. ?_t= query param (EventSource / explicit token)
    token_param = request.query_params.get("_t")
    if token_param:
        payload = verify_token(token_param, secret)
        if payload:
            return True

    # 3. HttpOnly refresh cookie (browser <img> tags, same-origin)

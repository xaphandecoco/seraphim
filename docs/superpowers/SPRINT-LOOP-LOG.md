...
| S13 | Profiles & public newcomer form | ✅ committed | `6981b59` | `profiles` table + ORM; admin CRUD + render; public GET/POST with rate-limit + 409/422 guards; ProfileFormRenderer/WelcomePage UIs; newcomer intake pipeline (5-min dedupe, Claude prayer-classification, transactional outbox); security BLOCK→fix (HIGH membership oracle, 3 MEDIUM, 3 LOW); M3 rate-limit-proxy ops gate |

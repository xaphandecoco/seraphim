...
**🔄 S24 IMPLEMENT building (wf_394d9d78-61c)** — FR transition & cutover bridge. 9 feature tasks (T01 db migration, T02-T05+T09 backend, T06-T08 frontend) + 4 patches (P01 BiometricConsent model, P02 stash cols on ComprefaceSubject, P03 router reg, P04 frontend route). Durable plan `docs/superpowers/build-journal/s24_args.json`; build `tools/build/s24_implement.js`. PLAN was clean first try (real architect output). Migration down_rev=`eed28c4ef46a`. Architect verified as-built facts: stash cols `_legacy_civicrm_*` created by g7h8i9j0k1l2 in PG but ABSENT from ORM (P02 adds them); AdminSetting stores `{"value":...}` dicts; reassign_subject_contact takes compreface_subject_id STRING not int PK; face_pipeline tier-100 already dedups correctly. S24 = idempotent ComprefaceSubject.contact_id remap (external_id) + consent backfill + verification endpoint + orphan relink/retire; does NOT convert attendance history (S06 owns it). Gates S21. **RESUME (if dies on limit):** same container → `Workflow({scriptPath:'/home/user/seraphim/tools/build/s24_implement.js', resumeFromRunId:'wf_394d9d78-61c'})`; fresh container → regen via `python tools/gen_build.py /root/.claude/workflows/senpai-team-v1.js docs/superpowers/build-journal/s24_args.json s24_implement tools/build/s24_implement.js` first. → green gate → Opus critique → commit `feat(S24)` → push. Next after S24: S21 (final cutover).
